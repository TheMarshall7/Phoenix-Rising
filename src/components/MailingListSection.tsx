 ...
 */
const defaultIframeId = `inline-${GHL_MAILING_LIST_FORM_ID}`;

export type MailingListSectionProps = {
  /** When set, shows pilgrimage waitlist copy (e.g. "Egypt", "France"). */
  pilgrimageName?: string;
};

export const MailingListSection: React.FC<MailingListSectionProps> = ({ pilgrimageName }) => {
  useGhlFormEmbed();
  ...
  return (
    <section
      id={isPilgrimage ? "waitlist" : undefined}
      className="relative py-32 md:py-44 px-6 sm:px-8 overflow-hidden border-y border-forest/[0.14] bg-gradient-to-b from-paper via-cream to-[#EDE8E0]"
    >
      {/* Depth & warmth */}
          ...
          <div className="grid gap-14 lg:grid-cols-12 lg:gap-0 lg:items-stretch">
            ...
            <div className="lg:col-span-5 flex flex-col justify-center pr-0 lg:pr-12 xl:pr-16 lg:border-r lg:border-forest/[0.08]">
              ...
              <div className="inline-flex items-center gap-3 text-[9px] tracking-[0.55em] uppercase font-semibold text-forest/55 mb-8">
                ...
                <span className="text-forest/70">{eyebrow}</span>
              </div>
              ...
              <p className="mt-8 text-lg md:text-xl font-light text-ink/55 leading-[1.85] tracking-wide max-w-md">
                {isPilgrimage ? (
                  <>
                    Be first to hear when the <strong className="font-medium text-ink/70">{pilgrimageName}</strong>{" "}
                    pilgrimage opens—dates, pricing, and how to reserve your place. Delivered with intention, never
                    noise.
                  </>
                ) : (
                  <>
